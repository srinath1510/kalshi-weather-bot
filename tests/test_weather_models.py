...
# Ensemble response with member temperatures
...
NWS_FORECAST_RESPONSE = {
    "properties": {
        "periods": [
            {
                "startTime": "2026-01-19T06:00:00-05:00",
                "isDaytime": True,
                "temperature": 45,
            },
            {
...
class TestOpenMeteoSource:
    ...
    def test_fetch_best_match_success(self):
        """Test successful fetch from best match endpoint."""
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
            json=OPEN_METEO_BEST_MATCH_RESPONSE,
            status=200,
        )

        source = OpenMeteoSource(NYC.lat, NYC.lon, NYC.timezone)
    ...
    def test_fetch_gfs_success(self):
        """Test successful fetch from GFS endpoint."""
        responses.add(
            responses.GET,
            OPEN_METEO_GFS_URL,
            json=OPEN_METEO_GFS_RESPONSE,
            status=200,
        )
    ...
    def test_fetch_ensemble_success(self):
        """Test successful fetch from ensemble endpoint with statistics."""
        ensemble_response = make_ensemble_response(TARGET_DATE, ENSEMBLE_TEMPS)
        responses.add(
            responses.GET,
            OPEN_METEO_ENSEMBLE_URL,
            json=ensemble_response,
            status=200,
        )
        ...
        expected_std = np.std(ENSEMBLE_TEMPS)
        expected_low = np.percentile(ENSEMBLE_TEMPS, 10)
        expected_high = np.percentile(ENSEMBLE_TEMPS, 90)

        assert abs(forecast.forecast_temp_f - expected_mean) < 0.01
        # std_dev should be at least MIN_STD_DEV (1.5)
        assert forecast.std_dev >= 1.5
        assert abs(forecast.low_f - expected_low) < 0.01
        assert abs(forecast.high_f - expected_high) < 0.01
        assert len(forecast.ensemble_members) == len(ENSEMBLE_TEMPS)

    @responses.activate
    def test_fetch_forecasts_all_sources(self):
        """Test fetch_forecasts returns results from all 3 endpoints."""
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
            json=OPEN_METEO_BEST_MATCH_RESPONSE,
        ...
        assert "Open-Meteo Best Match" in sources
        assert "GFS+HRRR" in sources
        assert "Open-Meteo Ensemble" in sources

    @responses.activate
    def test_missing_target_date(self):
        """Test handling when target date is not in response."""
        response_without_date = {
            "daily": {
                "time": ["2026-01-19", "2026-01-21"],
                "temperature_2m_max": [48.0, 55.0],
            }
        }
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
            json=response_without_date,
            status=200,
        )
    ...
    def test_null_temperature_value(self):
        """Test handling when temperature value is null."""
        response_with_null = {
            "daily": {
                "time": ["2026-01-19", "2026-01-20", "2026-01-21"],
                "temperature_2m_max": [48.0, None, 55.0],
            }
        }
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
            json=response_with_null,
            status=200,
        )
...
class TestNWSForecastSource:
    ...
    def test_fetch_forecast_success(self):
        """Test successful fetch from NWS API."""
        # Mock points endpoint
        responses.add(
            responses.GET,
            f"{NWS_API_BASE}/points/{NYC.lat},{NYC.lon}",
            json=NWS_POINTS_RESPONSE,
            status=200,
        )
        # Mock forecast endpoint
        responses.add(
            responses.GET,
            NWS_POINTS_RESPONSE["properties"]["forecast"],
            json=NWS_FORECAST_RESPONSE,
            status=200,
        )

        source = NWSForecastSource(NYC.lat, NYC.lon)
        forecasts = source.fetch_forecasts(TARGET_DATE)

        assert len(forecasts) == 1
        assert forecasts[0].source == "NWS"
        assert forecasts[0].target_date == TARGET_DATE
        assert forecasts[0].forecast_temp_f == 50.0

    @responses.activate
    def test_missing_target_date(self):
        """Test handling when target date is not in forecast periods."""
        responses.add(
            responses.GET,
            f"{NWS_API_BASE}/points/{NYC.lat},{NYC.lon}",
            json=NWS_POINTS_RESPONSE,
        ...
        # Forecast without target date
        forecast_without_date = {
            "properties": {
                "periods": [
                    {
                        "startTime": "2026-01-19T06:00:00-05:00",
    ...
    def test_points_api_error(self):
        """Test handling when points API fails."""
        responses.add(
            responses.GET,
            f"{NWS_API_BASE}/points/{NYC.lat},{NYC.lon}",
            status=500,
    ...
    def test_forecast_api_error(self):
        """Test handling when forecast API fails."""
        responses.add(
            responses.GET,
            f"{NWS_API_BASE}/points/{NYC.lat},{NYC.lon}",
            json=NWS_POINTS_RESPONSE,
    ...
    def test_only_returns_daytime_forecast(self):
        """Test that only daytime periods are returned."""
        responses.add(
            responses.GET,
            f"{NWS_API_BASE}/points/{NYC.lat},{NYC.lon}",
            json=NWS_POINTS_RESPONSE,
        ...
        # Forecast with only nighttime for target date
        nighttime_only = {
            "properties": {
                "periods": [
                    {
...
class TestCombinedWeatherSource:
    ...
    def test_fetch_from_all_sources(self):
        """Test that combined source fetches from all providers."""
        # Mock Open-Meteo endpoints
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
            json=OPEN_METEO_BEST_MATCH_RESPONSE,
        ...
        source = CombinedWeatherSource(NYC.lat, NYC.lon, NYC.timezone)
        forecasts = source.fetch_forecasts(TARGET_DATE)
    ...
    def test_partial_failure_still_returns_results(self):
        """Test that failing sources don't prevent other sources from returning."""
        # Only mock best match (others will fail)
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
            json=OPEN_METEO_BEST_MATCH_RESPONSE,
        ...
        # Should still have at least the best match forecast
        assert len(forecasts) >= 1
        assert forecasts[0].source == "Open-Meteo Best Match"
...
class TestFetchAllForecasts:
    ...
    def test_fetch_all_forecasts_nyc(self):
        """Test fetch_all_forecasts for NYC."""
        # Mock all endpoints
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
        ...
        forecasts = fetch_all_forecasts(TARGET_DATE, city="NYC")
    ...
    def test_fetch_all_forecasts_default_city(self):
        """Test fetch_all_forecasts uses NYC as default."""
        responses.add(
            responses.GET,
            OPEN_METEO_FORECAST_URL,
            json=OPEN_METEO_BEST_MATCH_RESPONSE,
...
class TestEnsembleStatistics:
    ...
    def test_ensemble_mean_calculation(self):
        ...
        ensemble_response = make_ensemble_response(TARGET_DATE, temps)
        responses.add(
            responses.GET,
            OPEN_METEO_ENSEMBLE_URL,
            json=ensemble_response,
        ...
        expected_mean = np.mean(temps)
        assert abs(forecast.forecast_temp_f - expected_mean) < 0.01

    @responses.activate
    def test_ensemble_percentiles(self):
        """Test that ensemble percentiles are calculated correctly."""
        temps = list(range(45, 65))  # 45-64, 20 values
        ensemble_response = make_ensemble_response(TARGET_DATE, temps)
        responses.add(
            responses.GET,
            OPEN_METEO_ENSEMBLE_URL,
        ...
        expected_p10 = np.percentile(temps, 10)
        expected_p90 = np.percentile(temps, 90)
        assert abs(forecast.low_f - expected_p10) < 0.01
        assert abs(forecast.high_f - expected_p90) < 0.01

    @responses.activate
    def test_min_std_dev_floor(self):
        """Test that MIN_STD_DEV floor is applied."""
        # Very tight ensemble with small spread
        temps = [55.0, 55.1, 55.2, 55.0, 55.1]  # std dev < 0.1
        ensemble_response = make_ensemble_response(TARGET_DATE, temps)
        responses.add(
            responses.GET,
            OPEN_METEO_ENSEMBLE_URL,
